...
public class OrderService {
    ...
    public String prepareOrder(int orderID, String clientUsername) {
        // Step 1: Find the order by orderID or clientUsername
        String findOrderSql = "SELECT p.pieceNum, l.roomNum, l.shelfNum " +
                "FROM Ordered o " +
        ...
        // Fetch the items for the given orderID or clientUsername
        ...
        // Step 2: Update the location of the items to the "holding location"
        String holdingLocationSql = "UPDATE PieceIn SET roomNum = ?, shelfNum = ?, notes = 'Ready for delivery' WHERE pieceNum = ?";
        for (ItemDetails item : items) {
            // Assume holding location is room 99, shelf 99 (example)
            jdbcTemplate.update(holdingLocationSql, 99, 99, item.getPieceNum());
        }

        // Step 3: Flag items as unavailable (if applicable)
